<template>
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="detail">
        <h5>商品清单</h5>
        <ul class="list clearfix" v-for="(orderDetail, index) in orderDetailList" :key="index">
          <li>
    ...
    <div class="trade">
      ...
      <div class="receiveInfo">
        ...
        收货人：<span>{{ destination.name }}</span>
...
export default {
  ...
  computed: {
    ...
    orderDetailList() {
      if (this.tradeInfo.detailArrayList) {
  ...
  methods: {
    changeAddress(address, addressList) {
      ...
      address.isDefault = '1';
      this.destination.address = address.fullAddress;
      this.destination.name = address.consignee;
      this.destination.phone = address.phoneNum;
    },
    submitOrder() {
      let data = {
        consignee: this.destination.name,
        consigneeTel: this.destination.phone,
        deliveryAddress: this.destination.address,
        paymentWay: 'ONLINE',
        orderComment: this.message,
        orderDetailList: this.orderDetailList,
      };
      reqsubmitOrder(this.tradeInfo.tradeNo, data).then((value) => {
        if (value.code == 200) {
          this.$router.push(`/pay?orderId=${value.data}`);
        } else {
          alert(value.message);
        }
      });
